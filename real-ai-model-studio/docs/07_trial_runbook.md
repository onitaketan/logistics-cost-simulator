...
- **AIエンジンは mock**（実画像は生成されない。生成フロー・状態遷移の確認用）。
- **生成は同期実行**（実エンジン導入時にキュー非同期化予定。API契約は据え置き設計）。
- **ストレージは local backend**（S3/R2 の SSE-KMS 本結線は未。生画像は署名URL経由のみ）。
- 本人/事務所の承認ポータル（外部ログイン）は未実装（P2）。代理承認として記録。
